class InitCasesComponent {
    ...
    makePageLayout() {
        ...
        leftArrow.setAttribute("id", "arrow-0");        
        rightArrow.setAttribute("id", "arrow-1");
...
class ProcessTableData {
    ...
    startProcessing() {
        ['casesPerOneMillion', 'deathsPerOneMillion', 'recoveredPerOneMillion'].forEach ((crit) => {
            this.infoByCountriesPer100Total.forEach((value) => {value[crit] = value[crit] / 10});
        });
        ['todayCases', 'todayDeaths', 'todayRecovered'].forEach ((crit) => {
            this.infoByCountriesPer100LastDay.forEach((value) => {value[crit] = value[crit] / (value.population / 100000)});
        });
    ...
    renderData() {        
        ...
        this.info.forEach((value) => {
            ...
            //div.setAttribute("id", `${value.countryInfo.iso2}`);
            divWrapper.appendChild(div);
            div = document.createElement("div");
